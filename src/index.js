...
const Private_Keys = require('./model/accounts');
...
(() => {
  ...
  // Set up request client for use through workflow
   ...
   */
    ...
    const Seed_Private_Key = Private_Keys[0];
		const recipients = [];
   ...
   */
    ...
    if (txCounters.initiated >= options.total) { return; }
    ...
    // Setting up the ongoing agent
    const timerId = setInterval(
			() => sendTransaction(transactionFactories[mode]).then(numSuccessfulTransactions => {
				if (numSuccessfulTransactions < options.total) { return; }
        console.log('exiting');
